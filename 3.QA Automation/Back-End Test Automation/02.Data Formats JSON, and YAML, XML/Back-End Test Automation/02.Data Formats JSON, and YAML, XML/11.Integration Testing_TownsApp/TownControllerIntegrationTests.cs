...
namespace TownApplication.IntegrationTests
{
    public class TownControllerIntegrationTests
    {
        ...
        public void AddTown_ValidInput_ShouldAddTown()
        {
            ...
            Assert.Equal("Plovdiv",town);
            Assert.Equal(500, population);
            Assert.True(population == 500);
            Assert.Equal(7, town.Length);

            var result = _controller.GetTownByName(town);
            Assert.NotNull(result);
            Assert.NotNull(town.Substring(0, 7));
        ...
        public void AddTown_DuplicateTownName_DoesNotAddDuplicateTown()
        {
            ...
            _controller.AddTown(town, population + 200);
            ...
            Assert.Equal(5005, population);
            Assert.Single(result);

            var item = result[0];
            Assert.Equal(population, item.Population);
            Assert.Equal(town, item.Name);
        }
        ...
        public void DeleteTown_ShouldDeleteTown()
        {
            ...
            var townName = "Stara Zagora";
            var population = 100300;
        ...
        public void ListTowns_ShouldReturnTowns()
        {
            ...
            var town1 = "Stara Zagora";
            var population1 = 100;
            _controller.AddTown(town1, population1);

            var town2 = "Yablanitza";
            var population2 = 200;
            _controller.AddTown(town2, population2);

            var town3 = "Teteven";
            var population3 = 300;
            _controller.AddTown(town3, population3);
            ...
            Assert.Equal(town1, result1.Name);
            Assert.Equal(town2, result2.Name);
            Assert.Equal(town3, result3.Name);
        }
    }
}
